...
class Workbook {
        ...
        this.addContextSheet();
            ...
            for (let t of types) {
                if (this.crate.types[t]) {
                    var sheetName = t.replace(/https?:\/\/.*\//ig, "").replace(/\W/g, "_");
                    if (t.length > 31) { 
                        const intLength = types.size.toString().length;
                        sheetName = `${sheetName.substring(0, 30 - intLength)}${typesArray.indexOf(t)}`;
                    } else {
                        sheetName = "@type=" + sheetName;
                    }
    ...
    isEmptyValue(val) {
        ...
        if (Array.isArray(val)) {
            return val.length === 0 || val.every(entry => this.isEmptyValue(entry));
        }
        return false;
    }
    ...
    parseCell(val) {
        var cellString = '';
        if (val && typeof val.value === "boolean") {
            return val.value;
        }
        if (val && typeof val.result === "boolean") {
            return val.result;
        }
        ...
        if (cellString === null || cellString === undefined) {
            return "";
        }
                    ...
                    if (columns[cell._column._number]) {
                        ...
                        if (prop && !prop.startsWith(".")) {
                            ...
                            if (prop === "@id") {
                                ...
                                const values = me.asArray(propValue);
                                for (let val of values) {
                                    item[prop].push({"@id": val})
                                }
                            } else if (prop.startsWith("isTerm_") && propValue) {   
                                console.log("isTerm", propValue, typeof propValue)
                                const values = me.asArray(propValue);
                                if (!item[prop.replace("isTerm_", "")]) {
                                        item[prop.replace("isTerm_", "")] = [];
                                }
                                for (let val of values) {
                                    const resolvedTerm = me.crate.resolveTerm(val);
                                    if (resolvedTerm) {
                                        item[prop.replace("isTerm_", "")].push({"@id": resolvedTerm});
                                    } else {
                                        item[prop.replace("isTerm_", "")].push({"@id": val});
                                    }
                                }
                                     }  else if (!me.isEmptyValue(propValue)) {
    ...
    resolveLinks() {
        ...
        for (let item of this.crate.entities()) {
            for (let prop of Object.keys(item)) {
                var vals = [];
                if (prop.startsWith("isReverse_")) {
                    for (let val of item[prop]) {
                        const target = this.crate.getEntity(val);
                        if (target) {
                            const propName = prop.replace("isReverse_", "");
                            this.crate.addValues(target, propName, item);
                        } else {
                            this.log.warning.push(`Item ${item["@id"]} has a reverse link to ${val} but it does not exist`);
                            console.log("Warning: Item", item["@id"], "has a reverse link to", val, "but it does not exist");
                        }
                    }
                    this.crate.deleteProperty(item, prop);
                } else for (let val of item[prop]) {
                     if (val && !val["@id"]) {
        ...
        // Cleanup pass: remove any isReverse_* properties that are empty
        for (let item of this.crate.entities()) {
            for (let prop of Object.keys(item)) {
                if (prop.startsWith("isReverse_")) {
                    const v = item[prop];
                    const empty = this.isEmptyValue(v);
                    if (empty) {
                        this.crate.deleteProperty(item, prop);
                    }
                }
    ...
    backLinkItem(item) {
        // Add backlinks only when they are not already present
                ...
                for (let i of item[p1]) {
                    if (i && i['@id']) {
                        ...
                        if (target) {
                            ...
                            if (target[p2]) {
                                const existing = Array.isArray(target[p2]) ? target[p2] : [target[p2]];
    ...
    addRdfsProps() {
        ...
        for (let item of this.crate.graph) {
            if (item['@type'].includes('rdf:Property') || (item['@type'].includes('rdfs:Class'))) {
                ...
                if (!item['rdfs:comment'] && item.description) {
                    console.log("Adding rdfs:comment to", item)
